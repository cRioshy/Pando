...
Der siebentägige Stock-Verification-Erfassungslauf wurde am 17. August 2026 nach dem finalen Snapshot beendet. Um 19:14 Uhr Europe/Berlin meldeten Plattform und alle 13 Services `OK`; die 7-Tage-API zeigte 19.100 Fälle mit 13.563 `COMPLETED`, 1.692 `PENDING` und 3.845 `UNKNOWN`. Der anschließend bei vollständig gestoppter Runtime gelesene Gesamtbestand umfasst 19.164 eindeutige Fälle, 55.131 gültige JSONL-Zeilen und null JSON-Fehler. Darin stehen 13.615 abgeschlossene, 1.692 offene und 3.857 unbekannte Fälle. 194 historische Verification-IDs besitzen je eine zusätzliche Completion-Zeile; diese History bleibt unverändert.

Der Abschluss-Branch `agent/verification-stabilization` ergänzt drei voneinander getrennte Sicherheitsänderungen. Die Verification besitzt nun die expliziten Modi `NORMAL`, `DRAIN` und `STOPPED`; unbekannte Modi bleiben ohne Subscriptions fail-closed. `DRAIN` akzeptiert keine neuen `STOCK_SHADOW_OBSERVED`-Fälle, darf aber bereits vorhandene Decision-/Tracker-Links und fällige Quote-Outcomes weiterführen. Das Ledger wird während der gesamten Adapterlebensdauer durch einen prozess- und OS-weiten exklusiven Lock geschützt; bei Lockkonflikt startet der Adapter ohne Subscriptions. Completion prüft unter derselben Adapter-Sperre erneut `PENDING` und führt Append plus Materialized-View-Update serialisiert aus. Der allgemeine simulierte `OutcomeTracker` verwendet für seine offene Zustandsdatei jetzt den vorhandenen konfliktresistenten `atomic_write_json`-Pfad, während Snapshot und Schreiben gemeinsam unter seiner Zustandssperre liegen. 47/47 gezielte und 325/325 vollständige Tests bestanden; Decision-, Confidence-, Gate-, Telegram-, Order- und Market-Regime-Logik wurden nicht verändert.

Der kontrollierte DRAIN-Smoke am 17. August 2026 lief mit genau einer Instanz, Verification `enabled=true`, öffentlichem Modus `DRAIN`, Telegram `enabled=false`/`dry_run=true`/`messages_sent=0`, `ready_for_telegram=false` und `order_execution_allowed=false`. Die kanonische Fallzahl und `VERIFICATION_CREATED` blieben exakt bei 19.164. 32 bereits vorhandene Fälle wurden abgeschlossen: `COMPLETED` stieg von 13.615 auf 13.647 und `PENDING` sank von 1.692 auf 1.660. Die 194 historischen Zusatz-Completions blieben unverändert; JSON-Fehler blieben null. Der anschließende kontrollierte Stop gab Port 8000 in 1,961 Sekunden frei, der Prozess endete vollständig und das Ledger blieb quieszent.

Das geprüfte AFTER-Backup liegt unter `C:\Users\Admin\Desktop\PandorickBackUp_2026-08-17_19-29-44_AFTER.zip`: 1.587.549.058 Byte, 1.313 Einträge beziehungsweise 1.312 Dateien und 11.763.410.095 unkomprimierte Byte. Der vollständige Stream-Test, die separator-normalisierte Pflichtprüfung und die Testextraktion von `.git/HEAD`, `AGENTS.md`, Zustandsdokument, Verification-Test und Adapterquelle bestanden. SHA-256: `E9378D017F235219FA78B84ACEA9DBB0EF738E7EFEADFBEBB9454A21B17B5CAD`. Das Backup enthält den getesteten Code und die bis dahin vorbereiteten Zustandsdokumente; die erst nach dem DRAIN-Smoke ergänzte Abschlussübergabe liegt im späteren Dokumentations-Commit.

Market Regime Contract v1 ist auf dem separaten Branch `agent/market-regime-contract-v1` observer-only implementiert. Die bestehende echte Crypto-`15m`-Serie und die öffentliche Stock-`1d`-Serie werden nur intern an eine begrenzte Queue übergeben. Der Worker validiert Feature Quality, berechnet drei voneinander unabhängige Achsen (Trendrichtung, Volatilität, Trendphase), persistiert kompakte restart-stabile Snapshots append-only und stellt read-only Coverage/API/UI bereit. Rohkerzen und vollständige Features verlassen den Worker nicht. Es besteht keine Verbindung zu Decision Core, Shadow Gate, Outcomes, Telegram oder Orders. 55/55 gezielte und 318/318 vollständige Tests bestanden. Ein isolierter Live-Smoke-Test persistierte BTCUSDT und AAPL in einem Batch ohne Drop oder Fehler, beendete Queue und Worker sauber und publizierte keine Decision-, Signal-, Shadow-, Trade-, Telegram- oder Orderereignisse. Der Test nutzte nur temporären Speicher und veränderte den laufenden Produktionsdienst nicht.

Das validierte AFTER-Backup liegt unter `C:\Users\Admin\Desktop\PandorickBackUp_2026-08-12_22-16-19_AFTER.zip`: 1.512.476.762 Byte, 1.201 Einträge, einschließlich `.git`, Dokumentation, Tests, `AGENTS.md` und Quellcode; .NET-Öffnung und vollständige Testextraktion waren erfolgreich.

PR #23 wurde nach erneuter Prüfung und ausdrücklicher Freigabe am 14. August 2026 per Merge-Commit `c751fe18e966dc6800d80925c8c7020093c85e8e` nach `main` integriert. Der vollständige Stock-/Polling-/Verification-Head `4f685522267b33277f1fa3da2444b132dd2cfbff` ist aus `origin/main` erreichbar. Draft-PR #24 ist jetzt direkt gegen `main` gerichtet, weiterhin offen, Draft und ungemergt. Der neue `main` wurde ohne Rebase/Force-Push in den Regime-Branch aufgenommen und der getestete Stand einschließlich Übergabe als `7c7d4f8f0598f0dc943a86b9ab4be53baba979ca` veröffentlicht; 318/318 Tests sowie Diff-, Syntax-, Scope- und Secret-Prüfungen bestanden in der abschließenden Wiederholung.

Am 12. August 2026 wurde der nach dem PC-Neustart reproduzierte Stock-/Polling-Freeze behoben. Der Produktions-Orchestrator wartet nicht mehr auf die vollständig blockierende Legacy-Stock-Pipeline: genau ein Hintergrundlauf bleibt erlaubt, sein Ergebnis wird in einem Folgetakt übernommen und parallele Legacy-Schreibvorgänge bleiben ausgeschlossen. Die gesamte Normalisierung einschließlich Yahoo-Quote, Tageskerzen, Feature Engine und Shadow liegt im Hintergrundlauf. Der zweite reale Engpass lag im synchronen Verification-Consumer: fällige 24h-Outcomes werden jetzt chronologisch, append-only und restart-safe in Batches von höchstens acht je Stock-Quote nachgearbeitet, statt tausende Ledgerzeilen in einem Event zu schreiben. 20/20 gezielte und 299/299 Gesamttests bestanden. Der finale Einzelprozess lief anschließend über drei Gesamtzyklen: Stock und Crypto jeweils drei Zyklen, keine STALE-Dienste, null Sitzungsfehler, Runtime-Stderr 0 Byte, Telegram aus/Dry-Run/null Sendungen und Verification-/Orderfreigaben weiterhin `false`.

Am 12. August 2026 wurde PandorickKi nach einem PC-Ausfall um 12:51 Uhr Europe/Berlin mit exakt dem freigegebenen siebentägigen Stock-Verification-Profil wieder gestartet. Der letzte vorhandene append-only Ledger-Zeitstempel vor dem Neustart war `2026-08-11T22:44:09.901899+00:00`, entsprechend einer lokalen Erfassungslücke von ungefähr zwölf Stunden und sieben Minuten; bestehende Daten blieben erhalten. Runtime-Preflight bestand mit Python 3.12.13. Health, Webserver und Verification-Service melden `OK`, Verification ist `enabled=true`, der Fingerprint bleibt `3d23f923d6b9d9dc3019457afcb078591b5d8c8b4d1f4f4db55911724fa71747`, Telegram bleibt aus/Dry-Run und Telegram-/Orderfreigaben bleiben `false`. Der erste öffentliche Stock-Zyklus schrieb neue Fälle, blieb bei der abschließenden Prüfung jedoch länger als drei Minuten im Status `RUNNING`; API und übrige Services reagierten weiter, Runtime-Stderr blieb leer. Dieser noch laufende Zyklus ist kein bestätigter Fehler, muss aber bis zum regulären Abschluss beobachtet werden.

Der ausdrücklich freigegebene siebentägige Stock-only Verification-Lauf ist seit 10. August 2026, 19:03:44 Uhr Europe/Berlin aktiv. Der vorherige normale Prozess wurde ausschließlich über die Stop-API geordnet beendet und Port 8000 vor dem Neustart freigegeben. Der Lauf verwendet 60-Sekunden-Zyklen, 24h-Forward-Mark-to-Market, 0,05-%-Neutralband und den unveränderten Konfigurationsfingerprint `3d23f923d6b9d9dc3019457afcb078591b5d8c8b4d1f4f4db55911724fa71747`. Nach zwei vollständigen Zyklen waren Health und alle zwölf Services `OK`; zehn neue Fälle und zehn Decision-Links wurden append-only geschrieben, Runtime-Stderr blieb leer. Zwölf beim Neustart ergänzte `OUTCOME_COMPLETED`-Einträge gehören zu den bereits am 9. August angelegten Kurzlauffällen und müssen bei der Abschlussauswertung zeitlich von den neuen Laufdaten getrennt werden. Verification ist `enabled=true`; Telegram bleibt aus/Dry-Run und sämtliche Telegram-/Order-/Active-Decision-Flags bleiben `false`. Geplantes kontrolliertes Ende ist der 17. August 2026 um 19:10 Uhr Europe/Berlin; der Abschluss darf keinen Fit und keine automatische Kalibrierung ausführen.

Der fachliche Vertrag `pandorickki.stock-shadow-calibration` Version 1 ist als reine Dokumentations- und Reviewgrenze definiert; seine Mindestabdeckungen wurden am 10. August 2026 ausdrücklich bestätigt. Es gibt noch kein Kalibrierungsmodul und kein Artefakt. Er trennt Rohscore, kalibrierte Erfolgswahrscheinlichkeit und Evidenz-Confidence, dedupliziert wiederholte Zyklen derselben Tageskerze und verlangt chronologische Fit-/Holdout-Trennung. Die heutige Marktphasenmessung besitzt trotz 20 berechneter Shadows null abgeschlossene unabhängige 24h-Verification-Outcomes, weil die Verification im normalen Betrieb deaktiviert war. Der aktuelle Status ist deshalb verbindlich `INSUFFICIENT_DATA`; keine Probability oder Confidence wird abgeleitet. Die Bestätigung startet keinen Verification-Lauf. Selbst ein später validiertes Artefakt bliebe stock-only/offline/observer-only und dürfte Gate, Telegram oder Orders nicht beeinflussen.

Am 10. August 2026 wurde der unveränderte observer-only Stock-Pfad während der geöffneten US-Marktphase von 18:05:44 bis 18:11:32 Uhr Europe/Berlin über fünf vollständige Stockzyklen (1205 bis 1209) read-only beobachtet. Die 25 Audits ergaben 15 `READY` und 10 `BLOCKED`. Aus 20 berechneten öffentlichen Shadows entstanden 10 LONG, 5 SHORT und 5 HOLD; 15 Risikopläne wurden berechnet und 10 blockiert. Die vier unterstützten Symbole lieferten in allen Zyklen Yahoo-Quotes mit 8,737 bis 19,505 Sekunden Alter; `SPCX` blieb in allen fünf Zyklen ohne öffentlichen Quote-Zeitstempel blockiert. Plattform und Stock-Service blieben gesund, Sitzungsfehler und STALE-Services null; vorübergehendes `RUNNING` einzelner Services war ausschließlich der normale parallele Zykluszustand. NeuroBrain blieb bei Queue 0, Drops 0 und Fehlern 0; das Fehlerjournal blieb unverändert bei 191 Ereignissen und null Schreibfehlern. Telegram blieb `enabled=false`, `dry_run=true`, `messages_sent=0`; Stock-Verification war deaktiviert und meldete `ready_for_telegram=false` sowie `order_execution_allowed=false`. Die aktive Stock-API-Projektion und stichprobenartig geprüfte neue kompakte Brain-/Decision-/Gate-Einträge enthielten keine `stock_shadow*`-, `stock_data_audit`- oder `stock_candle*`-Felder.

Die Stock-only Live-Shadow-Verification ist als optionaler, standardmäßig deaktivierter Observer implementiert. `StockAdapter` erzeugt vor dem unveränderten aktiven Stock-Ereignis eine kompakte interne `STOCK_SHADOW_OBSERVED`-Projektion mit derselben `source_event_id`. `StockShadowVerificationAdapter` verbindet darüber später die vorhandene deterministische `decision_id`, schreibt ausschließlich append-only Ledger-Recordtypen und rekonstruiert seinen Materialized View nach Neustarts aus aktiver Datei plus Archiven. Crypto wird ignoriert. Bestehende Legacy-, Decision-, Outcome-, Learning-, Telegram- und Orderpfade werden nicht verändert.

Outcome-Vertrag Version 1 ist ein festes 24-Stunden-Forward-Mark-to-Market mit 0,05-%-Neutralband und strikt fortgeschrittenem öffentlichen Quote-Zeitstempel. Legacy und Shadow werden getrennt klassifiziert; HOLD oder fehlende Daten bleiben `UNKNOWN`. Stop-/Zielberührungen werden nicht behauptet. API und Control Center liefern nur kompakte read-only Summary-/Detailprojektionen. Der kurze kontrollierte Lauf endete nach drei Zyklen selbstständig mit allen zwölf Diensten `OK`: 15 eindeutige Stockfälle, 15 Decision-Verknüpfungen, 12 `PENDING`, 3 erwartete `SPCX`-`UNKNOWN`, ein einheitlicher Konfigurationsfingerprint, keine Rohkerzen/`raw_result`, Telegram-/Order-/Active-Decision-Flags durchgehend `false`. Port 8000 war anschließend geschlossen; der siebentägige Lauf wurde nicht gestartet.

Der observer-only Vertrag `pandorickki.stock-shadow-risk` Version 1 ergänzt LONG-/SHORT-Shadow-Kandidaten um einen normalisierten Risikoplan aus derselben öffentlichen Sicht. Entry ist der öffentliche Kurs; die Distanz ist `max(ATR14 × 1,0; 0,5 % vom Entry)`, Stop liegt bei 1R und Ziele bei 1R/2R/3R. HOLD und ungültige Daten blockieren fail-closed. Alle Parameter sind explizit konfiguriert. Shadow, Risiko, Vergleich und Audit werden vor dem aktiven Stock-Abschlussereignis entfernt und weder publiziert noch persistiert. Der Datenvertrag darf sie nur intern auditieren; selbst `READY` bleibt ohne Telegram-/Orderwirkung. 37/37 gezielte und 289/289 Gesamttests bestanden. Zwei Livezyklen erzeugten 8 Shadows und 6 gültige LONG-/SHORT-Pläne; 2 HOLD- und 2 `SPCX`-Fälle blockierten korrekt. Die zehn Daten-Audits blieben am Sonntag wegen Quote-Freshness beziehungsweise fehlender Eignung sicher `BLOCKED`. Plattform/Services waren gesund, Sitzungsfehler und STALE null, Telegram aus/Dry-Run mit null Sendungen.

Am 9. August 2026 wurde `pandorickki.stock-shadow-candidate` Version 1 ergänzt. `StockAdapter` berechnet damit einen kompakten observer-only Kandidaten ausschließlich aus öffentlichen Yahoo-Tageskerzen und öffentlichem Kurs. Richtung und `probability` sind strikt von Legacy-Richtung und -Probability getrennt und werden in einer Vergleichssicht mit `affects_active_decision=false` ausgewiesen. Der Score ist transparent, deterministisch und als `UNVALIDATED_HEURISTIC_SCORE` gekennzeichnet, nicht als kalibrierte Wahrscheinlichkeit oder Confidence. Rohkerzen werden nicht publiziert oder persistiert. Der aktive Feature-/Decision-/Signalpfad bleibt unverändert; Risiko fehlt absichtlich, Telegram und Orders bleiben immer gesperrt. 31/31 gezielte und 283/283 Gesamttests bestanden. Nach kontrolliertem Neustart liefen zunächst zwei vollständige Zyklen mit Plattform und allen 11 Services gesund, Sitzungsfehlern und STALE-Services null. Acht öffentliche Shadow-Kandidaten wurden berechnet; alle zehn Stock-Audits blieben sicher `BLOCKED`. Nach der abschließenden Neutralitätskorrektur wurde der final getestete Stand erneut geordnet geladen und über einen weiteren vollständigen Zyklus mit denselben gesunden Sicherheitswerten geprüft. Telegram blieb aus/Dry-Run mit null gesendeten Nachrichten.
Grundlage: aktueller Arbeitsbaum, statische Codeprüfung, lokale HTTP-API und zuletzt tatsächlich ausgeführte Tests.

Am 9. August 2026 wurde der eigenständige Stock-Datenvertrag `pandorickki.stock-data` Version 1 ergänzt und anschließend als getrennter read-only Auditpfad in `StockAdapter` integriert. `StockCandleService` lädt bis zu 260 öffentliche Yahoo-Tageskerzen mit query1/query2-Fallback und 15-Minuten-In-Memory-Cache; die Kerzen werden nur lokal geprüft und niemals in Event-, Brain-, Decision- oder NeuroBrain-Payloads übernommen. Ein echter AAPL-Test lieferte 260/260 Kerzen mit `PASS/VERIFIED/READY`. Beim damaligen ersten Providerstand liefen vier Zyklen mit 20 sicheren Blockierungen, weil Richtung/Probability noch aus dem Legacy-Placeholder stammten und der öffentliche Risikoplan fehlte. Dieser historische Zwischenstand wurde inzwischen durch den getrennten öffentlichen Shadow- und Risikopfad abgelöst. Der aktive Feature-/Decision-/Signalpfad blieb und bleibt unverändert; Telegram und Orders können durch den Auditpfad nicht freigegeben werden.
...
- `adapters/stock_adapter.py`: Aktienanalyse mit entsprechender Feature-Anreicherung und derselben 500-Kerzen-Grenze; zusätzlich vollständig getrennter öffentlicher Shadow-Kandidat und Legacy-Vergleich ohne Einfluss auf aktive Decisions.
- `stock_shadow_candidate.py`: kompakter observer-only Aktienkandidat aus öffentlichen Tageskerzen und Kurs; unkalibrierter Version-1-Score, Risikoplan nur über den getrennten Shadow-Risikovertrag, keine Persistenz oder Freigabe.
- `stock_shadow_risk.py`: versionierter observer-only ATR14-Risikoplan aus der öffentlichen Shadow-Sicht; Entry, Stop und drei Ziele, fail-closed und ohne Event-/Freigabewirkung.
